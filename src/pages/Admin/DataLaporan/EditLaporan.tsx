...
export default function FormEditLaporan() {
  ...
  useEffect(() => {
    ...
    const fetchLaporanById = async () => {
      try {
        ...
        const { product_id, penjualan_id, tgl_laporan } = response.data.data;

        setProductId(product_id);
        setPenjualanId(penjualan_id);
        setTglLaporan(tgl_laporan);
      } catch (error) {
        console.error("Error : ", error);
      }
    };
  ...
  const editReport = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const token = localStorage.getItem("token");
      const response = await axios.put(
        `http://127.0.0.1:8000/api/admin/laporan/updateLaporan/${id}`,
        {
    ...
    <div className="flex h-screen bg-white">
      ...
      <div className="flex flex-1 flex-col">
        ...
        <div className="flex-1 p-6 overflow-y-auto mt-7">
          ...
          <h1 className="font-medium text-3xl">Form Edit Laporan</h1>
