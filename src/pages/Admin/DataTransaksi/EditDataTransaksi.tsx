...
export default function FormEditDataTransaksi() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchDataTransaksiById = async () => {
      try {
        const token = localStorage.getItem("token");
        const response = await axios.get(
          `http://127.0.0.1:8000/api/admin/transaksi/getDataTransaksiById/${id}`,
          {
            headers: {
        ...
        setPenjualanId(penjualan_id);
        setTglTransaksiMasuk(tgl_transaksi_masuk);
        setTglTransaksiKeluar(tgl_transaksi_keluar);
      } catch (error) {
    ...
    fetchDataTransaksiById();
    fetchProduk();
    fetchPenjualan();
  }, [id]);

  const editDataTransaksi = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const token = localStorage.getItem("token");
      const response = await axios.put(
        `http://127.0.0.1:8000/api/admin/transaksi/updateDataTransaksi/${id}`,
        {
      ...
      setMessage("Error, gagal mengubah data!");
      setShowModal(true);
      console.error("Error : ", error);
    }
  };
